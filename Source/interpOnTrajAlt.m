 function varOnTraj = interpOnTrajAlt(varToGet,y,rayGd,ncForRay,cnst)
 ...
 %      ncForRay  -  critical density for the ray in cm^-3
    ...
    [weights,inds] = rectLocate(x,rayGd.zVal,rayGd.rVal);% get rectangle
    % Thought - might add kLambadD to the below
    
    switch varToGet
      case 'valsNe'
        % to do - check to see if the variable exists
        triVals = triInterp(weights,inds,rayGd.gridNe);
        
      case 'valsDLogNedz'
        triVals = triInterp(weights,inds,rayGd.gridDLogNedz);%rayGd.valsDLogNedz(rayGd.DT(ti,:));
      case 'valsDLogNedr'
        triVals = triInterp(weights,inds,rayGd.gridDLogNedr);%rayGd.valsDLogNedr(rayGd.DT(ti,:));
      case 'valsTe'
        triVals = triInterp(weights,inds,rayGd.gridTe);%rayGd.valsTe(rayGd.DT(ti,:));
%       case 'valsTi'
