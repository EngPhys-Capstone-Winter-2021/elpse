...
%
 ...
 %
 halt = rayBundle.halt;
 trajs = rayBundle.trajs;
 waveType = rayBundle.type;
 rayICs = rayBundle.rayICs;
 
 frequency = rayBundle.frequency;
 n_c = rayBundle.nc;
 kdir = rayBundle.direction;
 ...
 parfor rayIdx = 1:rayBundle.nrays
     ...
     trajIsNew = isempty(trajs{rayIdx}); 
     
     
     if trajIsNew    % need IC's for trajectory
         if waveType == 'EM'
             x0(:) = rayICs(:,rayIdx)';  % position x0 is row vector here
             % modify arguments to "toDispSurface()" to only what is
             % needed
             %
             omega = frequency(rayIdx);
             nc = n_c(rayIdx);
             k0 = toDispSurface(x0,omega,nc,kdir,c,rayIdx,rayGd);         
             tPrev = 0;                         % start from t=0
         else
             error('Currently new trajectories are for EM waves only')
         end         
     else
         lastInfo = trajs{rayIdx}(end,:); % use the last time info
     ...
     %
     omega_ps = 1.e-12*frequency(rayIdx);
...
% $$$      end
     ...
     if tStop > tPrev
         switch waveType
           case 'EM'
             [tr,yr] = ode45(@(t,y) odeEmRayFun(t,y,omega_ps, ...
                                                rayGd,clum,ln10,twopi),tSpan,ray0);
           case 'EPW'
         ...
         if any(~withinMargin)
             %this needs to be
             halt(rayIdx) = 1;   % set halt flag for
                                           % out-of-margin
         end
         % attach solution to rayBundle structure
         %  have two cases here... 
         if trajIsNew
             trajs{rayIdx} = newTraj;
         else
             % append to existing
             oldTraj = trajs{rayIdx}(1:end-1,:); % drop IC
             trajs{rayIdx} = [oldTraj; newTraj];
             %             rayBundle.trajs{rayIdx} = newTraj;   % debug
         end
       end  % the if tSpan..
          
 end   % for loop over rays
 rayBundle.trajs = trajs;
 rayBundle.halt = halt;
 ...
 function k0 = toDispSurface(x0,omega,nc,kdir,c,rayIdx,rayGd)
 ...
 function dydt = odeEmRayFun(t,y,omega_ps,rayGd,clum,ln10,twopi)
 ...
 function dydt = odeLwRayFun(t,y,omega_ps,rayGd,clum,ln10,twopi)
