...
% $$$      end
 ...
 % (JFM 21 July, 2020)
    ...
    if goodPt
        ...
        [w_val,inds] = rectLocate(x,rayGd.uniqueZ,rayGd.uniqueR);
        logNe = triInterp(w_val,inds,rayGd.grid);     % log10 of electron density
                                         %    disp(logNe)  % debugging
        netonc = 10^(logNe)/nc;
        
        %triValDLogNedz = rayGd.valsDLogNedz(rayGd.DT(ti,:));
        dLogNedz = triInterp(w_val,inds,rayGd.gridZ);   % at phase space point
                                                %    disp(dLogNedz)

        %triValDLogNedr = rayGd.valsDLogNedr(rayGd.DT(ti,:));
        dLogNedr = triInterp(w_val,inds,rayGd.gridR);%dot(bc',triValDLogNedr')';   % at phase space point
                                                %    disp(dLogNedr)
