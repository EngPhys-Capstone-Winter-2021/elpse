...
function bundleOut = pushBundle_CustomRectK(rayBundle,rayGd,tStep,margin,npts)
% 
...
% $$$      end
 ...
 % (JFM 21 July, 2020)
    ...
    if goodPt
        ...
        logNe = rectInterp2d(x,rayGd.uniqueZ,rayGd.uniqueR,rayGd.grid);     % log10 of electron density
                                         %    disp(logNe)  % debugging
        netonc = 10^(logNe)/nc;
        
        %triValDLogNedz = rayGd.valsDLogNedz(rayGd.DT(ti,:));
        dLogNedz = rectInterp2d(x,rayGd.uniqueZ,rayGd.uniqueR,rayGd.gridZ);   % at phase space point
                                                %    disp(dLogNedz)

        %triValDLogNedr = rayGd.valsDLogNedr(rayGd.DT(ti,:));
        dLogNedr = rectInterp2d(x,rayGd.uniqueZ,rayGd.uniqueR,rayGd.gridR);%dot(bc',triValDLogNedr')';   % at phase space point
                                                %    disp(dLogNedr)
